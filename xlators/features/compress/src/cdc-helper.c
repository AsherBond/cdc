 ...
 * Data sent = <compressed-data> + <trailer>
 * The trailer contains the checksum.
 *
 * This header is added only during debugging.
 */
                              ...
                              int size)
{
        ...
        alloc_len = size ? size : priv->buffer_size;
...
cdc_init_zlib_output_stream (cdc_priv_t *priv, cdc_info_t *ci, int size)
{
        ...
        ci->stream.avail_out = size ? size : priv->buffer_size;
}

/* XXX: This routine is for debugging only (make sense during a
 *      single readv call)
 */
void
cdc_dump_iovec_to_disk (xlator_t *this, cdc_info_t *ci, const char *file)
{
        ...
        for (i = 0; i < ci->ncount; i++) {
                writen = write (fd, (char *) ci->vec[i].iov_base,
                                ci->vec[i].iov_len);
                gf_log (this->name, GF_LOG_DEBUG,
                        "dump'd %zu byte(s) to %s", writen + 10, GF_CDC_DEBUG_DUMP_FILE );
        }
                       ...
                       int flush)
{
        ...
        for (;;) {
                deflate_len = priv->buffer_size - ci->stream.avail_out;
                 ...
                 cdc_info_t *ci)
{
        ...
        while (ci->stream.avail_in != 0) {
                if (ci->stream.avail_out == 0) {
                        ...
                        CURR_VEC(ci).iov_len = priv->buffer_size;
...
/* This routine does the actual compression of data using
 * Zlib library routines. The compressed data is sent in
 * gzip format viz.
 * (gzip_header + compressed data + gzip_trailer)
 *
 * This header/trailer is an overhead for each request.
 * Going forward we may want to only send the raw compressed
...
static unsigned long
cdc_extract_crc (cdc_info_t *ci, char *trailer)
{
        return cdc_get_long ((unsigned char *) &trailer[0]);
}

static unsigned long
cdc_extract_size (cdc_info_t *ci, char *trailer)
{
...
do_cdc_decompress (xlator_t *this, cdc_priv_t *priv, cdc_info_t *ci)
{
        ...
        trailer = ((char *) vec.iov_base) + vec.iov_len
                - GF_CDC_VALIDATION_SIZE;

        /* CRC of uncompressed data */
        computed_crc = cdc_extract_crc (ci, trailer);

        /* size of uncomrpessed data */
        computed_len = cdc_extract_size (ci, trailer);
