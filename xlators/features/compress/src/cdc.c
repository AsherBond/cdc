 ...
 */
        ...
        STACK_UNWIND_STRICT (readv, frame, ci.nbytes, op_errno,
                             ci.vec, ci.ncount, stbuf, ci.iobref,
                             xdata);
...
init (xlator_t *this)
{
        ...
        GF_OPTION_INIT ("cdc-level", priv->cdc_level, int32, err);
        ...
        /* Set Gzip {in/de}flate Buffer Size */
        GF_OPTION_INIT ("buffer-size", priv->buffer_size, int32, err);
        if ( !priv->buffer_size ) {
                gf_log (this->name, GF_LOG_WARNING,
                        "Invalid buffer size, using the default");
                priv->buffer_size = GF_CDC_DEF_BUFFERSIZE;
        }
        ...
        if ( priv->min_file_size < 0 ) {
                gf_log (this->name, GF_LOG_WARNING,
                        "Invalid min file size, using the default");
                priv->min_file_size = GF_CDC_MIN_CHUNK_SIZE;
        }
...
struct volume_options options[] = {
        ...
        { .key  = {"buffer-size"},
          .default_value = "16384",
          .type = GF_OPTION_TYPE_INT,
          .description = "Size of internal buffer used by \
                          zlib library functions."
        },
        { .key  = {"mem-level"},
          .default_value = "8",
          .type = GF_OPTION_TYPE_INT,
